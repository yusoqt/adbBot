...
def workflow_reid_gear(device, thread_id, showdb=False, mode=1):
    """
    Workflow: Reid Gear
    ขั้นตอน: find_click -> find_click -> check_gear -> end
    ...
    try:
        ...
        print(f"[Thread {thread_id} - {device_serial}] 🚀 เริ่ม Workflow: Reid Gear (mode={mode})")
        ...
        load_img(device=device, pic_local='image/gacha.png', threshold_set=0.7, timeout=5, loop_image_delay=4)
        click(device=device, local=[50,180])

        load_img(device=device, pic_local='image/event/found.png', threshold_set=0.7, timeout=5, loop_image_delay=4)

        if (find_img_and_click(device=device, pic_local='image/event/ruby.png', threshold_set=0.85, timeout=15, loop_image_delay=2)) :
            delay(0.5)
            find_img_and_click(device=device, pic_local='image/event/buy.png', threshold_set=0.7, timeout=5, loop_image_delay=2)
            delay(0.5)
            find_img_and_click(device=device, pic_local='image/ok_button.png', threshold_set=0.7, timeout=5, loop_image_delay=2)
            delay(1)
            find_img_and_click(device=device, pic_local='image/ok_button.png', threshold_set=0.7, timeout=5, loop_image_delay=2)
            delay(2)
            click(device=device, local=[50,50])
        ...
        delay(1)
        find_img_and_click(device=device, pic_local='image/gear.png', threshold_set=0.7, timeout=15, loop_image_delay=4, region=[650, 0, 960, 250])
        delay(1)
        load_img(device=device, pic_local='image/gear_1times.png', threshold_set=0.7, timeout=1, loop_image_delay=4)
        # Gacha 1 pos: 150
        ...
        delay(3)
        find_img_and_click(device=device, pic_local='image/gear_1times.png', threshold_set=0.7, timeout=15, loop_image_delay=4)
        find_img_and_click(device=device, pic_local='image/ok_button.png', threshold_set=0.7, timeout=15, loop_image_delay=4)



        # == Step 10: Check Gear
        matched_gears = []   # list หลักเก็บผลลัพธ์
        ...
        while (True):
            if check_timeout():
                return {'success': False, 'message': 'Timeout exceeded'}
            print("check Gear")
            if (load_img(device=device, pic_local='image/random_again.png', threshold_set=0.7, timeout=25, loop_image_delay=4)):
                print(f"[Thread {thread_id}] 📝 เช็ค Gear...")
                
                temp_set = set()  # ใช้ set ชั่วคราวป้องกันค่าซ้ำในแต่ละรอบ
                
                gear_name = checkwant_gear(device, scan_times=15, region=[0, 0, 470, 470], showdb=True)

                if gear_name:
                    print(f"[Thread {thread_id}] ✅ เจอ Gear: {gear_name}")
                    temp_set.add(gear_name)   # add ลง set ชั่วคราว
                    delay(1)

                # เอาจาก set ไปใส่ list หลัก
                matched_gears.extend(temp_set)

                find_img_and_click(device=device, pic_local='image/random_again.png', threshold_set=0.7, timeout=3, loop_image_delay=4)
                
            if (load_img(device=device, pic_local='image/check_ruby.png', threshold_set=0.7, timeout=5, loop_image_delay=4)):
                find_img_and_click(device=device, pic_local='image/ok_button.png', threshold_set=0.7, timeout=15, loop_image_delay=4)
            else:
                break
        ...
        while (True):
            if check_timeout():
                return {'success': False, 'message': 'Timeout exceeded'}
            if (find_img_and_click(device=device, pic_local='image/close.png', threshold_set=0.7, timeout=4, loop_image_delay=4, region=[0, 0, 920, 540])):
                delay(0.5)
                continue
            else: break
        ...
        with clipboard_lock:
            max_retries = 2
            id = None
            load_img(device=device, pic_local='image/copy_success.png', threshold_set=0.7, timeout=8, loop_image_delay=4)
        ...
        if matched_gears:
            namefile = f"{'_'.join(matched_gears)}_{id}_LINE_COCOS_PREF_KEY.xml"
        else:
